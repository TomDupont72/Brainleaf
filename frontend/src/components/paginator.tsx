import { Pagination, PaginationContent, PaginationItem, PaginationLink, PaginationNext, PaginationPrevious, PaginationEllipsis } from "@/components/ui/index";
    ...
    return (
        <Pagination className="pb-3 flex justify-center">
            <PaginationContent>
                ...
                { currentPage > 1 && currentPage < pageNumber ? (<PaginationItem>
                    <PaginationLink isActive>{currentPage}</PaginationLink>
                </PaginationItem>) : null }
                { currentPage < pageNumber - 1 ? (<PaginationItem>
                    <PaginationLink onClick={() => setCurrentPage(currentPage + 1)}>{currentPage + 1}</PaginationLink>
                </PaginationItem>) : null }
                { currentPage < pageNumber - 2 ? (<PaginationItem>
                    <PaginationEllipsis />
                </PaginationItem>) : null }
                { pageNumber > 1 ? (<PaginationItem>
                    <PaginationLink isActive={currentPage === pageNumber} onClick={() => setCurrentPage(pageNumber)}>{pageNumber}</PaginationLink>
                </PaginationItem>) : null }
                <PaginationItem>
